    ...
    * @dev The node must also verify it doesn't use banned opcodes, and that it doesn't reference storage outside the wallet's data.
    ...
    external returns (uint256 preOpGas, uint256 prefund, address actualAggregator, bytes memory sigForUserOp, bytes memory sigForAggregation, bytes memory offChainSigInfo) {
        ...
        if (actualAggregator != address(0)) {
            sigForUserOp = IAggregator(actualAggregator).validateUserOpSignature(userOp);
        }
    ...
    unchecked {
        ...
        try IPaymaster(paymaster).validatePaymasterUserOp{gas : gas}(op, opInfo.requestId, requiredPreFund) returns (bytes memory _context, uint256 validationData){
            context = _context;
        } catch Error(string memory revertReason) {
    ...
    function getNonce(address sender, uint192 key) public view override(IEntryPoint, NonceManager) returns (uint256 nonce) {
